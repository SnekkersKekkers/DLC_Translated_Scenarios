MsgType(1);
BGOpen("ex980",0);
ChLayout(0);
ScrFadeIn(0);
VoicePlay("T021140000_11_000");
MsgDisp("Onari?","So this is how it turned out...");
MsgClear();
Wait(60);
MsgDisp("主人公","Huh... Onari-sensei?∋");
SEPlay("EV_SE_FOOT_WALK_COME_ALONE");
SEWait();
BGMPlay("BGM_C11_ONARI_KOKU",0.01);
ChOpen(11,254,0,0,0,#1,#1,0,0);
VoicePlay("T021140000_11_010");
MsgDisp("Onari","Hey, congratulations on graduating.");
MsgDisp("主人公","Thank you.
But what are you doing here?");
ChEye(11,0);
ChMouth(11,0);
ChMotion(11,4);
VoicePlay("T021140000_11_020");
MsgDisp("Onari","I pestered Osako-sensei until he let me in.
As for why I ended up in the chapel...
I wonder why.");
MsgDisp("主人公","Hehe, that's so like you, Onari-sensei.");
ChEye(11,0);
ChMouth(11,4);
ChMotion(11,2);
VoicePlay("T021140000_11_030");
MsgDisp("Onari","Is it?
If you say so, then I suppose it is...");
MsgDisp("主人公","...Onari-sensei?");
ChEye(11,0);
ChMouth(11,4);
ChMotion(11,4);
VoicePlay("T021140000_11_040");
MsgDisp("Onari","Ever since I was a kid, I never understood
what it meant to have a sense of self.");
ChEye(11,2);
ChMouth(11,4);
ChMotion(11,2);
ChEyeOpenLevel(11,0);
VoicePlay("T021140000_11_050");
MsgDisp("Onari","Not just myself, but other people too.
What makes someone who they are... that was
always hard for me...");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,0);
VoicePlay("T021140000_11_060");
MsgDisp("Onari","Someone like that wanting to be a teacher,
and aiming to be like Osako-sensei of all people —
he feels like the most out-of-reach person in the world to me.");
MsgDisp("主人公","But... you were always trying so hard,
working toward your ideal
of what a teacher should be.");
ChEye(11,1);
ChMouth(11,4);
ChMotion(11,2);
ChEyeOpenLevel(11,0);
VoicePlay("T021140000_11_070");
MsgDisp("Onari","Yeah, back then I thought I could get there.
So I'd imitate Osako-sensei and practice
introducing myself in the park.");
MsgDisp("主人公","Seeing you like that...
it gave me a push forward too.");
ChEye(11,4);
ChMouth(11,0);
ChMotion(11,0);
VoicePlay("T021140000_11_080");
MsgDisp("Onari","Because you said that, I kept going for the
teaching exam without giving up.");
ChEye(11,4);
ChMouth(11,0);
ChMotion(11,4);
VoicePlay("T021140000_11_090");
MsgDisp("Onari","We met by chance, and without even realizing
it, we were helping each other.");
MsgDisp("主人公","I... didn't do anything...");
ChEye(11,2);
ChMouth(11,4);
ChMotion(11,0);
ChEyeOpenLevel(11,0);
VoicePlay("T021140000_11_100");
MsgDisp("Onari","Without our cram school, I still wouldn't
believe I could be useful to anyone.");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,5);
VoicePlay("T021140000_11_110");
MsgDisp("Onari","That time we spent together — it was
something I needed more than anyone.");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,4);
VoicePlay("T021140000_11_120");
MsgDisp("Onari","You patiently taught me, over and over again.
What only I could do.");
MsgDisp("主人公","And because of you, Onari-sensei,
I started thinking seriously about the future —
about becoming a wonderful teacher...");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,0);
VoicePlay("T021140000_11_130");
MsgDisp("Onari","I wonder what kind of teacher
you'll become...");
MsgDisp("主人公","I want to be like you, Onari-sensei.");
ChEye(11,4);
ChMouth(11,0);
ChMotion(11,3);
VoicePlay("T021140000_11_140");
MsgDisp("Onari","Didn't I say you're already there?
Just look at how you are with my students —
it's plain to see.");
ChEye(11,4);
ChMouth(11,0);
ChMotion(11,0);
VoicePlay("T021140000_11_150");
MsgDisp("Onari","More than that — you're the one
who made me a teacher.");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,4);
VoicePlay("T021140000_11_160");
MsgDisp("Onari","No... you made me human.");
MsgDisp("主人公","Human...");
ChEye(11,2);
ChMouth(11,4);
ChMotion(11,0);
VoicePlay("T021140000_11_170");
MsgDisp("Onari","It finally happened.
I can genuinely feel afraid now.
Being with you — it makes me afraid.");
MsgDisp("主人公","What?");
ChEye(11,2);
ChMouth(11,4);
ChMotion(11,4);
VoicePlay("T021140000_11_180");
MsgDisp("Onari","I realized it at the sky lounge.
If something happened to you... just thinking
about it frightened me...");
MsgDisp("主人公","Onari-sensei...");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,0);
VoicePlay("T021140000_11_190");
MsgDisp("Onari","For the first time, I have something truly
precious.");
MsgClear();
ChClose(11);
ChCustomLayout(11,5,0,-2.5);
Wait(60);
SEPlay("EV_SE_544");
ChOpen(11,254,0,4,4,#1,#1,0,0,0,120);
VoicePlay("T021140000_11_200");
MsgDisp("Onari","That's you.");
ChEye(11,4);
ChMouth(11,4);
ChMotion(11,0);
VoicePlay("T021140000_11_210");
MsgDisp("Onari","I love you.");
